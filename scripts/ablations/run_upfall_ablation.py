...
Usage:
    ...
    python scripts/ablations/run_upfall_ablation.py
    
    # Single condition
    python scripts/ablations/run_upfall_ablation.py --condition depth_lora
    
    # With temporal shuffle test
    python scripts/ablations/run_upfall_ablation.py --test_shuffle
"""
...
def main():
    ...
    # Run conditions
    conditions = [args.condition] if args.condition else ["rgb", "depth_lora", "depth_zeroshot"]
    results = {}
    ...
    if len(results) == 2:
        delta_acc = results["depth_lora"]["test_accuracy"] - results["rgb"]["test_accuracy"]
        delta_f1 = results["depth_lora"]["test_f1"] - results["rgb"]["test_f1"]
        print(f"{'Δ (depth-rgb)':15s} | Acc: {delta_acc:+.4f} | F1: {delta_f1:+.4f}")
